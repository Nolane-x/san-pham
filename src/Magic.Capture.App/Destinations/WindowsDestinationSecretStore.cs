...
internal sealed class WindowsDestinationSecretStore : IDestinationSecretStore
{
    private const string Resource = "Magic Capture Desktop Destinations";
    ...
    public Task SaveAsync(string secretId, string value)
    {
        ...
        try
        {
            foreach (var existing in vault.FindAllByResource(Resource).Where(c => c.UserName == secretId).ToArray()) vault.Remove(existing);
        }
    ...
    public Task DeleteAsync(string secretId)
    {
        ...
        {
            var vault = new PasswordVault();
            foreach (var credential in vault.FindAllByResource(Resource).Where(c => c.UserName == secretId).ToArray()) vault.Remove(credential);
        }
        catch (Exception ex) when (CredentialVaultErrorPolicy.IsElementNotFound(ex))
        {
